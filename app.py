...
with col2:
    ...
    if st.button("🚨 Analyze Transaction", type="primary", use_container_width=True):
        ...
        # Force fraud for high-risk cases
        high_risk_conditions = (
            (amount > 5000 and location == "International") or
            (device == "Unknown Device" and amount > 3000) or
            (time_of_day == "Late Night (12AM - 6AM)" and location == "International") or
            (frequency == "Suspicious Pattern" and amount > 2000)
        )
        
        if high_risk_conditions:
            prediction = 1
            fraud_probability = 0.95
        elif manual_risk > 0.45:
            prediction = 1
            fraud_probability = max(manual_risk, model_prob)
        else:
